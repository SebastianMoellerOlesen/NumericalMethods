...
def problem_c() -> None:
    ...
    for diff, order in zip(diffs, names):
        print()
        print(
            f"The maximum absolute error for the {order} error, is given by {np.max(np.abs(diff)):f}"
        )
...
def problem_d() -> None:
    ...
    for grid_count in grid_counts:
        ...
        # We create a 7xgrid_count matrix, which is more effecient that rolling each iter, and splicing the rolled array, as done it problem_c()
        # This way we create all the values we need, where each column is the i_-3 ... i_+3 values we need.
